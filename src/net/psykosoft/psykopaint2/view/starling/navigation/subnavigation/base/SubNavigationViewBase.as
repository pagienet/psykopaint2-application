package net.psykosoft.psykopaint2.view.starling.navigation.subnavigation.base
{
	...
	public class SubNavigationViewBase extends StarlingViewBase
	{
		...
		public function SubNavigationViewBase( title:String ) {
			...
			_frontLayer.addChild(_rightButtonContainer= new Sprite());
			_frontLayer.addChild(_leftButtonContainer= new Sprite());
		}
		...
		private function onHeaderResized( event:Event ):void {
			...
			TweenLite.to(_header,0.3,{x:- _header.width,ease:Expo.easeOut,onComplete:function(){
				_header.x=stage.stageWidth;
				TweenLite.to(_header,0.3,{x:stage.stageWidth / 2 - _header.width / 2,ease:Expo.easeOut});
			}});
		...
		protected function setLeftButton( label:String ):void {
			...
			TweenLite.to(_leftButtonContainer,0.10,{x:-300,ease:Expo.easeIn,onComplete:function(){
				TweenLite.to(_leftButtonContainer,0.15,{delay:0.3,x:0,ease:Expo.easeOut});
			}});
		...
		protected function setRightButton( label:String ):void {
			...
			TweenLite.to(_rightButtonContainer,0.10,{x:300,ease:Expo.easeIn,onComplete:function(){
				TweenLite.to(_rightButtonContainer,0.15,{x:0,ease:Expo.easeOut});
			}});
		}
...
//			_scrollContainer.backgroundSkin = new Quad( 100, 100, 0x222222 ); // Uncomment for visual debugging.
			...
			TweenLite.to(_scrollContainer,0.3,{x:- _scrollContainer.width,ease:Expo.easeIn,onComplete:function(){
				_scrollContainer.x = 1024 ;
				TweenLite.to(_scrollContainer,0.3,{x:1024 / 2 - _scrollContainer.width / 2,ease:Expo.easeOut});
			}});
