...
//			trace( ">>> fps: " + _fps );
	...
	public function runUiTests():void {
		...
		this.graphics.drawRect( 200, 200, bg.width, bg.height );
		this.graphics.endFill();*/
	...
	/*	var b1:SbButton = new SbButton();
		...
		var bg:ButtonGroup = new ButtonGroup();
		bg.addButton(b1); // Posiciona horizontalmente.
		bg.addButton(b2);
		bg.addButton(b3);
		bg.addButton(b4);
		bg.addButton(b5);
		bg.addButton(b6);
		bg.addButton(b7);
		bg.setButtonWithLabelSelected("LALALA");
		trace( bg.selectedBtnLabel(), "CURRENTLY SELECTED BTN" );

		var scroller:HItemScroller = new HItemScroller();
		scroller.x = 200;
		scroller.y = 200;
		addChild(scroller);
		scroller.addChild(b8);
