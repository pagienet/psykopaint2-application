package net.psykosoft.psykopaint2.core.drawing.brushkits
{
	...
	public class BrushKit_Paintgun extends BrushKit
	{
		...
		override protected function init( xml:XML ):void
		{
			...
			brushEngine.param_shapes.stringList = Vector.<String>(["inksplats","splats","spray","vectorsplat","crayon","paintbrush","sumi","paint1","basic","basic","basic"]);
		...
		protected function processPoints(points:Vector.<SamplePoint>, manager:PathManager, fingerIsDown:Boolean):Vector.<SamplePoint>
		{
			...
			for ( var i:int = 0; i < points.length; i++ )
			{
				...
				p.angle = Math.PI*0.5;// Math.atan2(rotationCenterY -p.y ,rotationCenterX - p.x) +  forceRotationAngle;
			}
