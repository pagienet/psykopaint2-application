	...
	public class BookView extends Sprite
	{
		...
		public function setSourceImages(sourceImageCollection:SourceImageCollection,shuffle:Boolean=false):void
		{
			...
			for (var i:int = 0; i < numberOfPages; i++) {
				...
				var newBookLayoutSamplesView:BookLayoutSamplesView = new BookLayoutSamplesView(_stage3DProxy);
				newBookLayoutSamplesView.imageSelected.add(onSourceImageSelected);	// simply forward the signal
		...
		public function setGalleryImageCollection(galleryImageCollection:GalleryImageCollection):void
		{
			...
			for (var i:int = 0; i < numberOfPages; i++) {
				...
				newBookLayoutGalleryView.imageSelected.add(onGalleryImageSelected);	// simply forward the signal
				currentBookPageView.addLayout(newBookLayoutGalleryView);
			}

		}

		private function onSourceImageSelected(proxy:SourceImageProxy):void
		{
			// make sure we're not triggering one while we were thumbing through pages
			if (_swipeMode == UNDECIDED)
				sourceImageSelected.dispatch(proxy);
		}

		private function onGalleryImageSelected(proxy:GalleryImageProxy):void
		{
			if (_swipeMode == UNDECIDED)
				galleryImageSelected.dispatch(proxy);
		}
